...
public class ControllerExceptionHandler {
  ...
  public ResponseEntity<ApiError> noHandlerFoundException(
    ...
    log.error("No handler found for request: {}", req.getRequestURI());
    ApiError apiError =
        new ApiError(
            "route_not_found",
   ...
   *
   * @param req the incoming request. the exception thrown when route is not found.
   *            {@link ResponseEntity} with 405 status code and the route that was not found in the
   */
  ...
  public ResponseEntity<ApiError> httpRequestMethodNotSupportedException(
    ...
    log.error("Method not allowed for request: {}", req.getRequestURI());
    ApiError apiError =
        new ApiError(
            "method_not_allowed",
            String.format("Method %s not allowed for route %s", req.getMethod(),
                req.getRequestURI()),
